namespace Win {
  ...
  let restingPlayer2Animation: Image[] = Utils.flipAnimation(
    Utils.swapAnimationColors(assets.animation`Pirate Rest`, 14, 4)
  );
  let coinPile: Sprite;
  let island: Sprite;
  let waves: Sprite[] = [];
  ...
  export function init() {
    ...
    restingPlayer2.y = 67;
    restingPlayer2.z = 3;
    animation.runImageAnimation(restingPlayer1, restingPlayer1Animation, 500, true);
    pause(100);
    animation.runImageAnimation(restingPlayer2, restingPlayer2Animation, 500, true);

    Utils.getArrayOfLength(10).forEach(() => {
      const wave = sprites.create(assets.animation`wave`[0]);
      animation.runImageAnimation(wave, waveAnimation, 500, true);
