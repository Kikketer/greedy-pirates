class EnemyPirate extends Enemy {
  ...
  static walkLeftAnimation: Image[] = Utils.flipAnimation(
    Utils.swapAnimationColors(assets.animation`Pirate Walk`, 14, 2)
  );
  static attackRightAnimation: Image[] = Utils.swapAnimationColors(assets.animation`Pirate Swing w Sword`, 14, 2);
  static attackLeftAnimation: Image[] = Utils.flipAnimation(
    Utils.swapAnimationColors(assets.animation`Pirate Swing w Sword`, 14, 2)
  );
  static deathRightAnimation: Image[] = Utils.swapAnimationColors(assets.animation`Pirate Dead`, 14, 2);
  static deathLeftAnimation: Image[] = Utils.flipAnimation(
    Utils.swapAnimationColors(assets.animation`Pirate Dead`, 14, 2)
  );
  static parryRightAnimation: Image[] = Utils.swapAnimationColors([assets.animation`Pirate Swing w Sword`[0]], 14, 2);
  static parryLeftAnimation: Image[] = Utils.swapAnimationColors(
    Utils.flipAnimation([assets.animation`Pirate Swing w Sword`[0]]),
    14,
    2
  );
  ...
  constructor({ x, y, target, riches }: { x: number; y: number; target?: EnemyTarget; riches?: number }) {
    super({
      ...
      y,
      target,
      sprite: sprites.create(assets.animation`Pirate Walk`[0]),
      riches,
      speed: 20,
  ...
  public render() {
    ...
    // Attack randomly
    if (
      control.millis() - this._lastAttackTick > this._nextAttackTime &&
  ...
  protected walk(direction?: "left" | "right") {
    ...
    if (this._facing === "left") {
      animation.runImageAnimation(this.sprite, EnemyPirate.walkLeftAnimation, 500, true);
    } else {
      animation.runImageAnimation(this.sprite, EnemyPirate.walkRightAnimation, 500, true);
    }
  }
  ...
  protected attack() {
    ...
    if (this._facing === "right") {
      animation.runImageAnimation(this.sprite, EnemyPirate.attackRightAnimation, 100, false);
    } else {
      animation.runImageAnimation(this.sprite, EnemyPirate.attackLeftAnimation, 100, false);
    }
